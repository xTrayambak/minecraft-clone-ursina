...
class World:
    def __init__(self):
        ...
        self.seed = random.randint(-9999999, 99999999)
                ...
                for Z in range(_ch_z, _ch_z + chunk.CHUNK_WIDTH):
                    for Y in range(chunk.CHUNK_HEIGHT):
                        for X in range(_ch_x, _ch_x + chunk.CHUNK_LENGTH):
                            ...
                            nX = self.noise.noise3(X/SCALE, Z/SCALE, self.seed)*AMP
                            nY = self.noise.noise3(Y/SCALE, X/SCALE, self.seed)*AMP
                            nZ = self.noise.noise3(Z/SCALE, Y/SCALE, self.seed)*AMP
                            
                            density = nX + nY + nZ + Y
                            
                            if density < 20:
                                print(f"Block placing at ({X}, {Y}, {Z}). Density={density}")
                                current_chunk.blocks[Z][Y][X] = 2
